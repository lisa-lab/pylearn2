...
class GaussianBandit(Environment):
    ...
    at initialization time from N(0, 1). (For the standard deviation we
    use the absolute value of the Gaussian sample)
    """

    def __init__(self, num_arms):
        self.rng = np.random.RandomState([2013, 11, 12])
        self.means = sharedX(self.rng.randn(num_arms))
        self.stds = sharedX(np.abs(self.rng.randn(num_arms)))
