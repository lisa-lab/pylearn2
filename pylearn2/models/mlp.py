...
class Sigmoid(Linear):
    ...
    def kl_matrix(self, Y, Y_hat):
        """
        .. todo::

            WRITEME properly

        Returns a matrix of KL divergences for each unit of each example
    ...
    def kl(self, Y, Y_hat):
        ...
        See comment for Sigmoid.kl_matrix() for description of the KL
        divergence and the way to compute it for binary values.

        """
        
        total = self.kl_matrix(Y=Y, Y_hat=Y_hat)
    ...
    def cost(self, Y, Y_hat):
        """
        Computes the global cost given targets and layer outputs
        
        Parameters
        ----------
        Y : symbolic variable representing the targets of the sigmoid
        Y_hat : symbolic variable representing the output of a sigmoid
        
        Y_hat must be generated by fprop, i.e., it must be a symbolic sigmoid.

        Returns
        -------
        output : theano scalar
            Global cost given provided targets and layer outputs
        """

        return self.cost_from_cost_matrix(self.cost_matrix(Y=Y, Y_hat=Y_hat))
    ...
    def cost_matrix(self, Y, Y_hat):
        ...
        -------
        output : theano matrix
            Matrix containing elementwise KL divergences between Y and Y_hat
        """

        return self.kl_matrix(Y=Y, Y_hat=Y_hat)
        
    @wraps(Layer.cost_from_cost_matrix)
    def cost_from_cost_matrix(self, cost_matrix):
        """
        Computes the global cost from a cost matrix
        
        Parameters
        ----------
        cost_matrix : matrix containing the costs associated with
                      outputs of a sigmoid layer
