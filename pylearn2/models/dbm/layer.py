...
class BinaryVector(VisibleLayer):
    ...
    def recons_cost(self, V, V_hat_unmasked, drop_mask = None, use_sum=False):
        """
        Returns the cost of reconstructing `V` as `V_hat`

        Parameters
        ----------
        V : theano matrix
            A design matrix of binary features
        V_hat_unmasked : theano matrix
            A design matrix of mean field predictions of V
        drop_mask : theano matrix
            Mask specifying which examples are inputs and which are targets
        use_sum : WRITEME
        """
        if use_sum:
            raise NotImplementedError()
    ...
    def recons_cost_per_example(self, V, V_hat_unmasked, drop_mask = None, use_sum=False):
        """
        Returns the cost of reconstructing `V` as `V_hat`
        ...
        if masked_cost.ndim != 2:
            raise NotImplementedError()

        return masked_cost.mean(axis=1)
...
class Softmax(HiddenLayer):
    ...
    def recons_cost_per_example(self, Y, Y_hat_unmasked, drop_mask_Y, scale):
        """
        The per-example cost of reconstructing `Y` as `Y_hat`. Specifically,
        the negative log probability.
