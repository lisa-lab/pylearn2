...
class RNN(Model):
    ...
    def gprop(self, state_below, n_steps=None):

        # Each batch will be provided with
        # shape = [batch_size, time_step, dimension]
        # Therefore, apply dimshuffle to let the scan function
        ...
        it0 = state_below[0]
        h0 = T.alloc(np.cast[config.floatX](0),
                     self.batch_size,
                     self.sum_dim)
        if self.batch_size == 1:
            it0 = T.unbroadcast(it0, 0)
            h0 = T.unbroadcast(h0, 0)
        if self.lstm:
            ...
            fn = lambda h, c, it: self.gprop_lstm_step(h, c, it)
            ((h, c, it, y), updates) = theano.scan(fn=fn,
                                                   outputs_info=[h0,
                                                                 c0,
                                                                 it0,
                                                                 None],
            ...
            fn = lambda h, it: self.gprop_step(h, it)
            ((h, it, y), updates) = theano.scan(fn=fn,
                                                outputs_info=[h0,
                                                              it0,
                                                              None],
                                                n_steps=n_steps)
    ...
    def gprop_step(self, state_before, iterator):

        rval = iterator
        if isinstance(self.layers[0].input_space, Conv2DSpace):
            rval = rval.dimshuffle('x', 0, 1, 'x')

        cnt = 0
        for layer in self.layers:
            if isinstance(layer, Recurrent):
                rval = layer.gprop(rval, state_before[:, self.rdim[cnt]])
                if cnt == 0:
                    rval_before = rval
                else:
                    rval_before = T.concatenate([rval_before, rval], axis=1)
                cnt += 1
            else:
                rval = layer.fprop(rval)

        if self.nvis == self.layers[-1].dim:
            iterator = T.set_subtensor(iterator[:], rval)
        else:
            iterator = T.alloc(T.concatenate([iterator[:, self.layers[-1].dim:],
                                              rval],
                                             axis=1),
                               self.batch_size,
                               self.nvis)
        if cnt == 0:
            rval_before = state_before

        #if self.batch_size == 1:
        #    iterator = T.unbroadcast(iterator, 0)
        #    rval_before = T.unbroadcast(rval_before, 0)
        #    rval = T.unbroadcast(rval, 0)

        return rval_before, iterator, rval

    def gprop_lstm_step(self, state_before, cell_before, iterator):
        ...
        for layer in self.layers:
            if isinstance(layer, Recurrent):
                rval, cell = layer.gprop_lstm(rval,
                                              state_before[:, self.rdim[cnt]],
                                              cell_before[:, self.rdim[cnt]])
                if cnt == 0:
                    rval_rec = rval
                    cell_rec = cell
                else:
                    rval_rec = T.concatenate([rval_rec, rval], axis=1)
                    cell_rec = T.concatenate([cell_rec, cell], axis=1)
                cnt += 1
            else:
                rval = layer.fprop(rval)
        ...
        if cnt == 0:
            rval_rec = state_before

        #if self.batch_size == 1:
        #    iterator = T.unbroadcast(iterator, 0)
        #    rval_rec = T.unbroadcast(rval_rec, 0)
        #    cell_rec = T.unbroadcast(cell_rec, 0)
        #    rval = T.unbroadcast(rval, 0)

        return rval_rec, cell_rec, iterator, rval
...
class Recurrent(Linear):
    ...
    def gprop(self, state_below, state_before):

        if state_below.ndim != 2:
            state_below = self.mlp.reformat(state_below, dimshuffle_on=True)
        assert state_below.ndim == 2
        z = T.dot(state_below, self.W) + T.dot(state_before, self.U)
        if self.use_bias:
            z += self.b
        z = T.tanh(z)
    ...
    def gprop_lstm(self, state_below, state_before, cell_before):
