...
class TestBinary:
    ...
    def test_binary_vis_layer_make_state(self):
        ...
        TestBinary.check_binary_samples(value, (num_samples, n), mean, tol)

    def test_binary_vis_layer_sample(self):
...
class TestGaussian:
    ...
    def test_gaussian_vis_layer_make_state(self):
        """
        ...
        TestGaussian.check_gaussian_samples(value, num_samples, n, rows, cols, channels, mean, tol)

    def test_gaussian_vis_layer_make_state_conv(self):
        """
        Verifies that GaussianVisLayer.make_state creates
        a shared variable whose value passes check_gaussian_samples
    ...
    def test_gaussian_vis_layer_sample(self):
        """
    ...
    def test_gaussian_vis_layer_sample_conv(self):
        """
        Verifies that GaussianVisLayer.sample returns an expression
        whose value passes check_gaussian_samples.
...
class TestBVMP:
    ...
    def test_bvmp_make_state(self):
        ...
        for pool_size in [1, 2, 5]:
            ...
            layer = BinaryVectorMaxPool(detector_layer_dim=n,
                                        layer_name='h',
                                        irange=1.,
                                        pool_size=pool_size)
            ...
            mean = layer.mf_update(state_below=T.alloc(0., 1, 1),
                                   state_above=None,
                                   layer_above=None)
            ...
            state = layer.make_state(num_examples=num_samples, numpy_rng=rng)

            value = [elem.get_value() for elem in state]

            TestBVMP.check_bvmp_samples(value, num_samples, n, pool_size, mean, tol)
    ...
    def make_random_basic_binary_dbm(
        ...
        h1 = BinaryVectorMaxPool(detector_layer_dim = num_h1,
        ...
        h2 = BinaryVectorMaxPool(center = center,
    ...
    @staticmethod
    def check_mfenergy(pool_size_1, rng, pool_size_2=None, center=False):

        # Make DBM and read out its pieces
        dbm = TestBVMP.make_random_basic_binary_dbm(rng = rng,
    ...
    @staticmethod
    def check_mfsamples(pool_size_1, rng, theano_rng, num_samples, tol):

        # Make DBM and read out its pieces
        dbm = TestBVMP.make_random_basic_binary_dbm(
            rng = rng,
            pool_size_1 = pool_size_1,
            )
        ...
        TestBVMP.check_bvmp_samples(h1_samples, num_samples,
                                    num_h, pool_size_1,
                                    (expected_p, expected_h), tol)

    def test_bvmp_mf_energy_consistent(self):
        ...
        for pool_size in [1, 2, 5]:
            TestBVMP.check_mfenergy(pool_size, rng, None, center=False)


    def test_bvmp_mf_energy_consistent_center(self):
        """
        A test of the BinaryVectorMaxPool class
        Verifies that the mean field update is consistent with
        ...
        """
        rng = np.random.RandomState([2012,11,1,613])
        TestBVMP.check_mfenergy(1, rng, 1, center=True)

    def test_bvmp_mf_sample_consistent(self):
...
class TestSoftmax:
    ...
    @staticmethod
    def check_mfenergy(rng, center=False):
        # Make DBM
        num_vis = rng.randint(1,11)
        ...
        y = Softmax(n_classes = n_classes,
                    layer_name = 'y',
                    irange = 1., center=center)
        y.set_biases(rng.uniform(-1., 1., (n_classes,)).astype(config.floatX), recenter=center)
    ...
    @staticmethod
    def check_mfsamples(rng, theano_rng, num_samples, tol):
        #Make DBM
        num_vis = rng.randint(1,11)
        n_classes = rng.randint(1, 11)
        ...
        TestSoftmax.check_multinomial_samples(y_samples, (num_samples, n_classes), expected_y, tol)

    def test_softmax_make_state(self, n=5, num_samples=1000, tol=0.04):
        ...
        TestSoftmax.check_multinomial_samples(value, (num_samples, n), mean, tol)

    def test_softmax_mf_energy_consistent(self):
        ...
        TestSoftmax.check_mfenergy(rng)

    def test_softmax_mf_energy_consistent_centering(self):
        ...
        rng = np.random.RandomState([2012,11,1,1131])
        TestSoftmax.check_mfenergy(rng, center=True)

    def test_softmax_mf_sample_consistent(self):
        ...
        num_samples = 1000
        tol = .042
        TestSoftmax.check_mfsamples(rng, theano_rng, num_samples, tol)


#TODO(devon): This needs a home
...
#TODO(devon): this test doesn't appear to do anything yet
...
#TODO(devon): Not sure what to do with this
def test_extra():
    """
