...
def rbm_ais(rbm_params, n_runs, visbias_a=None, data=None, 
    ...
    """
    Implements Annealed Importance Sampling for Binary-Binary RBMs, as described in:
    Neal, R. M. (1998) ``Annealed importance sampling'', Technical Report No. 9805 (revised),
    Dept. of Statistics, University of Toronto, 25 pages
    Ruslan Salakhutdinov , Iain Murray, On the quantitative analysis of deep belief networks,
    Proceedings of the 25th international conference on Machine learning, p.872-879, July
    05-09, 2008, Helsinki, Finland

    Parameters
    ----------
    rbm_params: list
        list of numpy.ndarrays containing model parameters: [weights,visbias,hidbias]
    n_runs: int
        number of particles to use in AIS simulation (size of minibatch)
    visbias_a: numpy.ndarray
        optional override for visible biases. If both visbias_a and data are None, visible
        biases will be set to the same values of the temperature 1 model. For best results,
        use the `data` parameter.
    data: matrix, numpy.ndarray
        training data used to initialize the visible biases of the base-rate model (usually
        infinite temperature), to the log-mean of the distribution (maximum likelihood solution
        assuming a zero-weight matrix). This ensures that the base-rate model is the "closest"
        to the model at temperature 1.
    betas: numpy.ndarray
        vector specifying inverse temperature of intermediate distributions (in increasing
        order). If None, defaults to AIS.dflt_betas
    key_betas: numpy.ndarray
        if not None, AIS.run will save the log AIS weights for all temperatures in `key_betas`.
        This allows the user to estimate logZ at several temperatures in a single pass of AIS.
...
def rbm_z_ratio(rbmA_params, rbmB_params, n_runs, v0=None,
    ...
    ----------
    rbmA_params: list
        list of numpy.ndarrays, corresponding to parameters of RBM whose partition Z_a is
        usually known (i.e. baserate model at beta=0). Parameters are given in the order:
        [weights, visbias, hidbias]
    rbmB_params: list
    other: 
        see rbm_ais description.
    """
    ...
    ### given current sample `v_sample`, generate new samples from inv. temperature `beta`
    new_v_sample = rbm_ais_gibbs_for_v(rbmA_params, rbmB_params, beta, v_sample)
    ...
    free_energy_fn = theano.function([beta, v_sample], fe, allow_input_downcast=False)
...
def rbm_ais_pk_free_energy(rbmA_params, rbmB_params, beta, v_sample):
    """
    Computes the free-energy of visible unit configuration `v_sample`, according to the
    interpolating distribution at temperature beta. The interpolating distributions are given
    by: p_a(v)^(1-beta) p_b(v)^beta. See equation 10, of Salakhutdinov & Murray 2008.
    ...
    v_sample: tensor.matrix
        matrix whose rows indexes into the minibatch, and columns into the data dimensions.

    Returns
    -------
    f : float (scalar)
       free-energy of configuration `v_sample` given by the interpolating distribution at
       temperature beta.
    """
    ...
    def rbm_FE(rbm_params, v, b):
        ...
        fe = - vis_term - tensor.sum(tensor.log(1 + tensor.exp(hid_act)), axis=1)
        return fe
...
def rbm_ais_gibbs_for_v(rbmA_params, rbmB_params, beta, v_sample, seed=23098):
    ...
    -----------
    rbmA_params: list
        Parameters of the baserate model (usually infinite temperature). List should be of
        length 3 and contain numpy.ndarrays corresponding to model parameters (weights,
        visbias, hidbias).
    rbmB_params: list
    ...
    ph_a = nnet.sigmoid((1-beta) * (tensor.dot(v_sample,weights_a) + hidbias_a))
    ha_sample = theano_rng.binomial(size=(v_sample.shape[0],len(hidbias_a)), n=1, p=ph_a, dtype=floatX)
    
    # equation 16 (Salakhutdinov & Murray 2008)
    ph_b = nnet.sigmoid(beta  * (tensor.dot(v_sample,weights_b) + hidbias_b))
    hb_sample = theano_rng.binomial(size=(v_sample.shape[0],len(hidbias_b)), n=1, p=ph_b, dtype=floatX)
    ...
    new_v_sample = theano_rng.binomial(size=(v_sample.shape[0],len(visbias_b)), n=1, p=pv, dtype=floatX)
...
class AIS(object):
    """
    Compute the log AIS weights to approximate a ratio of partition functions. After being
    initialized, the user should call the run() method which will compute the log AIS weights.
    estimate_from_weights will then compute the mean & variance of log(Zb/Za) (Eq.11).

    The notation used here is slightly different than in Salakhutdinov & Murray 2008. We write
    the AIS weights as follows (note that the denominator is always of the form p_i(x_i) to
    indicate that x_i ~ p_i.)
    ...
    def fX(a): return numpy.asarray(a,dtype=floatX)
    ...
    def __init__(self, sample_fn, free_energy_fn, v_sample0, n_runs, log_int=500):
        """
        ...
        sample_fn: compiled theano function, sample_fn(beta, v_sample) 
            returns new model samples, at inverse temperature `beta`.  Internally, we do this
            by performing block gibbs sampling using Eq.(15-17) (implemented in
            rbm_ais_gibbs_for_v) starting from configuration v_sample.
        free_energy_fn: compiled theano function, free_energy_fn(beta, v_sample)
            Computes the free-energy of of configuration v_sample at the interpolating
            distribution p_a^(1-beta) p_b^(beta).
        v_sample0: numpy.ndarray
            initial samples from model A.
        n_runs: int
            number of AIS runs (i.e. minibatch size)
        log_int: int
            log standard deviation of log ais weights every `log_int` temperatures.
        """
        ...
        ais_w = tensor.vector()
        dlogz = tensor.log(tensor.mean(tensor.exp(ais_w - tensor.max(ais_w)))) + tensor.max(ais_w)
    ...
    def run(self, n_steps=1):
        ...
        log_w^i += fe_{k-1}(v_{k-1}) - fe_{k}(v_{k-1}) recursively for all temperatures.
        """
        if not hasattr(self, 'betas'):
            self.set_betas()

        self.std_ais_w = []  # used to log std of log_ais_w at regular intervals
        self.logz_beta = []  # used to log log_ais_w at every `key_beta` value
        self.var_logz_beta = [] # used to log variance of log_ais_w at every `key_beta` value
        ...
        for i in range(len(self.betas)-1):
            ...
            # whenever we reach a "key" beta value, log log_ais_w and var(log_ais_w) so we can
            # estimate log_Z_{beta=key_betas[i]} after the fact.
    ...
    def estimate_from_weights(self, log_ais_w=None):
        """
        Once run() method has been called, estimates the mean and variance of log(Zb/Za).

        Parameters
        ----------
        log_ais_w: None or 1D numpy.ndarray
            optional override for log_ais_w. When None, estimates log(Zb/Za) using the log
            AIS weights computed by AIS.run() method.

        Returns
        -------
        f: float
            Estimated mean of log(Zb/Za), log-ratio of partition functions of model B and A.
        f: float
