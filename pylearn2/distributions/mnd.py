...
class MND:
    def __init__(self, sigma, mu, seed = 42):
        ...
        if not ( len(mu.shape) == 1 ):
            raise Exception('mu has shape '+str(mu.shape)+' (it should be a vector)')

        self.sigma_inv = solve(self.sigma, N.identity(mu.shape[0]), sym_pos = True)
    ...
    def free_energy(self, X):
        ...
        return .5 * T.sum( T.dot(  X - self.mu, T.dot( self.sigma_inv, T.transpose(X-self.mu))))
    #
    ...
    def random_design_matrix(self,m):
        Z = self.s_rng.normal( size = (m,self.mu.shape[0]), avg=0., std=1., dtype=floatX  )
