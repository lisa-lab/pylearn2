...
def foveate_channel(img, rings, output, start_idx):
    ...
    inner_img = img[:,
                    ring_w:img.shape[1] - ring_w,
                    ring_w:img.shape[2] - ring_w]
...
def defoveate_channel(img, rings, dense_input, start_idx):
    ...
    inner_img = dense_input[
        :, start_idx: end_idx].reshape(-1, inner_h, inner_w)
...
def restore_ring(output, coord, width, dense_input, start_idx):
    ...
    start_idx = restore_rect(output,
                             ...
                             start_idx)
    # right column, full height
    start_idx = restore_rect(output, coord,
                             img_w - coord - width,
                             img_h - coord,
                             img_w - coord,
                             width,
...
def restore_rect(output,
    ...
    for i in xrange(start_row, stop_row - width + 1, width):
        for j in xrange(start_col, stop_col - width + 1, width):
            ...
            output[:, i:i + width, j:j + width] = dense_input[
                :, idx][:, None, None]
            idx += 1
...
def get_encoded_size(img_h, img_w, rings):
    ...
    for r in rings:
        if (img_h % r) != 0 or (img_w % r) != 0:
            raise ValueError('Image width (%i) or height (%i) is not a '
                             'multiple of ring size %i' %
                             (img_h, img_w, r))
...
class RetinaCodingViewConverter(DefaultViewConverter):
    ...
    def __init__(self, shape, rings):
        ...
        self.topo_space = Conv2DSpace(
            shape=[rows, cols], num_channels=channels)
