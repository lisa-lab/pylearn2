...
class ICMP07DataSet(DenseDesignMatrix):
    """ Base class for ICML07 datasets.
    
    All these datasets can be displayed as 28x28 pixel datapoints.
    """
    def __init__(self, npy_filename, which_set, splits, one_hot):
        """
        """
        ...
        super(ICMP07DataSet, self).__init__(X = data_x, y = data_y, view_converter = view_converter)
    
    def get_test_set(self):
        return self.__class__(which_set='test', splits=splits, one_hot=one_hot)
...
class MNIST_rotated_background(ICMP07DataSet):
...
class Rectangles(ICMP07DataSet):
...
class RectanglesImage(ICMP07DataSet):
    """ ICML07: Discrimination between tall and wide rectangles.
